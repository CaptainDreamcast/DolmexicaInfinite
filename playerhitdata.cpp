...
static struct {
	map<int, PlayerHitData> mPassiveHitDataMap;
	map<int, PlayerHitData> mActiveHitDataMap;
	map<int, PlayerHitOverrides> mHitOverrideMap;
} gPlayerHitData;

static void loadHitDataHandler(void* tData) {
	(void)tData;
	gPlayerHitData.mPassiveHitDataMap.clear();
	gPlayerHitData.mActiveHitDataMap.clear();
	gPlayerHitData.mHitOverrideMap.clear();
}
...
static void updateSinglePlayerOverrides(void* tCaller, PlayerHitOverrides& tData) {
	(void)tCaller;
...
static void updateHitDataHandler(void* tData) {
	(void)tData;
	stl_int_map_map(gPlayerHitData.mHitOverrideMap, updateSinglePlayerOverrides);
}
...
{
	PlayerHitData passive;
	passive.mIsActive = 0;
	passive.mPlayer = tPlayer;

	int id = stl_int_map_push_back(gPlayerHitData.mPassiveHitDataMap, passive);

	PlayerHitData active;
	active.mIsActive = 0;
	active.mPlayer = tPlayer;

	gPlayerHitData.mActiveHitDataMap[id] = active;

	PlayerHitOverrides hitOverrides;
	int i;
	for (i = 0; i < 8; i++) hitOverrides.mHitOverrides[i].mIsActive = 0;

	gPlayerHitData.mHitOverrideMap[id] = hitOverrides;
...
void removePlayerHitData(DreamPlayer* tPlayer)
{
	...
	assert(stl_map_contains(gPlayerHitData.mHitOverrideMap, tPlayer->mHitDataID));

	gPlayerHitData.mActiveHitDataMap.erase(tPlayer->mHitDataID);
	gPlayerHitData.mPassiveHitDataMap.erase(tPlayer->mHitDataID);
	gPlayerHitData.mHitOverrideMap.erase(tPlayer->mHitDataID);
}
...
void copyHitDataToActive(DreamPlayer* tPlayer, void * tHitData)
{
	...
	assert(stl_map_contains(gPlayerHitData.mActiveHitDataMap, tPlayer->mHitDataID));
	PlayerHitData* active = &gPlayerHitData.mActiveHitDataMap[tPlayer->mHitDataID];
...
{
	if (!stl_map_contains(gPlayerHitData.mPassiveHitDataMap, tPlayer->mHitDataID)) return 0;
...
{
	if(!stl_map_contains(gPlayerHitData.mActiveHitDataMap, tPlayer->mHitDataID)) return 0;
...
{
	assert(stl_map_contains(gPlayerHitData.mHitOverrideMap, tPlayer->mHitDataID));
	PlayerHitOverrides* overrides = &gPlayerHitData.mHitOverrideMap[tPlayer->mHitDataID];
